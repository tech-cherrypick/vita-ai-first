...
const WeightTrendChart: React.FC<{ logs: WeekLogEntry[] }> = ({ logs }) => {
    ...
    return (
        <div className="p-4 bg-gray-50 rounded-lg mb-4">
             <h4 className="font-semibold text-gray-700 mb-2">Weight Trend (kg)</h4>
            ...
            <div className="flex justify-between text-xs text-gray-500 mt-1">
                <span>Week {logs[0].week} ({logs[0].weight}kg)</span>
                <span>Week {logs[logs.length-1].week} ({logs[logs.length-1].weight}kg)</span>
            </div>
        </div>
    );
};
                ...
                {sortedLogs.map(log => (
                    <div key={log.week} className="p-4 bg-white rounded-lg border border-gray-200">
                        <div className="flex justify-between items-baseline mb-2">
                             ...
                             <p className="text-sm font-semibold text-gray-800">Weight: {log.weight} kg</p>
                        </div>
                        <div>
