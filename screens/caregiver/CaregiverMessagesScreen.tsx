...
import { mockCareCoordinatorMessageThreads, MessageThread } from '../../constants';

interface CareCoordinatorMessagesScreenProps {
    initialSelectedThreadId?: string | null;
}

const CareCoordinatorMessagesScreen: React.FC<CareCoordinatorMessagesScreenProps> = ({ initialSelectedThreadId }) => {
    const [threads] = useState<MessageThread[]>(mockCareCoordinatorMessageThreads);
    ...
    const leadText: Record<MessageThread['lead'], string> = {
        bot: 'vita assist',
        careCoordinator: 'Care Coordinator (You)',
        doctor: 'Doctor'
    };
    ...
    const senderStyles: Record<any, string> = {
        ...
        careCoordinator: 'bg-brand-cyan text-white rounded-br-lg self-end',
        doctor: 'bg-brand-purple/10 text-brand-text rounded-bl-lg self-start',
    ...
    return (
        <div className="animate-fade-in">
            <div className="mb-8">
                ...
                <p className="mt-1 text-gray-600">Communicate with your patients and escalate to doctors when needed.</p>
            </div>
            ...
            <div className="flex flex-col md:flex-row h-[70vh] bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
                <aside className={`w-full md:w-1/3 border-r border-gray-200 flex-col ${isMobileThreadVisible ? 'hidden md:flex' : 'flex'}`}>
                    <div className="p-4 border-b">
                        <input type="text" placeholder="Search messages..." className="w-full px-3 py-2 text-sm bg-gray-100 border-transparent rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-cyan" />
                    </div>
                        ...
                        {threads.map(thread => (
                            <li key={thread.id}>
                                <button
                                    ...
                                    className={`w-full text-left p-4 hover:bg-gray-50 ${selectedThread?.id === thread.id ? 'bg-brand-cyan/5' : ''}`}
                                >
                        ...
                        <>
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <p className="text-xs text-gray-500">
                                            Next Action By: <span className={`font-bold ${selectedThread.lead === 'careCoordinator' ? 'text-red-600 animate-pulse' : 'text-brand-cyan'}`}>{leadText[selectedThread.lead]}</span>
                                        </p>
                                    </div>
                                </div>
                                 <button 
                                    onClick={() => alert(`This conversation has been escalated to ${selectedThread.patientId === 1 ? 'Dr. Sarah Mitchell' : 'Dr. Emily Rodriguez'}.`)}
                               ...
                               <div className="flex flex-col gap-3">
                                    {selectedThread.messages.map((msg, index) => (
                                        <div key={index} className={`max-w-[80%] p-3 rounded-2xl text-sm ${senderStyles[msg.sender]}`}>
                                            {msg.text}
                                            <p className={`text-xs mt-1 text-right ${msg.sender === 'careCoordinator' ? 'text-cyan-100' : 'text-gray-500'}`}>{msg.timestamp}</p>
                                        </div>
                                    ))}
                                </div>
                            </div>
                            <footer className="p-4 border-t bg-white">
                                <div className="flex items-center gap-2">
                                    <input type="text" placeholder="Type a message..." className="w-full px-4 py-2 bg-gray-100 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-cyan" />
                                    <button className="px-4 py-2 text-sm font-semibold text-white bg-brand-cyan rounded-lg hover:opacity-90">Send</button>
                                </div>
                            </footer>
                        </>
                    ) : (
