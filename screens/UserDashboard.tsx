...
import { Patient, TimelineEvent, DashboardView } from '../constants';
...
const TreatmentProgressSection: React.FC<{ patient: Patient; onUpdatePatient: any }> = ({ patient, onUpdatePatient }) => {
    ...
    return (
        <div id="treatment-progress-section" className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden mb-8 transition-all duration-300">
            <button
...
const MedicationSummary: React.FC<{ patient: Patient }> = ({ patient }) => (
    <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm">
        <h3 className="text-lg font-bold text-gray-900 mb-4 flex items-center gap-3">
            <span className="w-8 h-8 rounded-lg bg-brand-pink/10 flex items-center justify-center text-sm">💊</span>
            Current Regimen
        </h3>
        <div className="space-y-4">
            <div className="p-4 bg-gray-50 rounded-2xl border border-gray-100">
                <div className="flex justify-between items-center mb-1">
                    <span className="font-bold text-gray-900">Compound GLP-1</span>
                    <span className="text-xs font-bold text-brand-purple bg-brand-purple/10 px-2 py-0.5 rounded-full">Active</span>
                </div>
                <p className="text-xs text-gray-500">Dosage: 0.25mg • Next dose in 3 days</p>
            </div>
            <div className="p-4 bg-gray-50 rounded-2xl border border-gray-100">
                <div className="flex justify-between items-center mb-1">
                    <span className="font-bold text-gray-900">MuscleProtect BCAA</span>
                    <span className="text-xs font-bold text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">Daily</span>
                </div>
                <p className="text-xs text-gray-500">1 scoop with breakfast</p>
            </div>
        </div>
    </div>
);
        ...
        <div className="fixed top-6 right-6 z-[100] animate-slide-in-right">
            <div className="bg-white rounded-[24px] shadow-2xl border border-brand-purple/20 p-5 flex items-center gap-5 max-w-sm">
                ...
                <div className="flex-1">
                    <p className="text-[10px] font-black uppercase tracking-widest text-brand-purple mb-1">Incoming Session</p>
                    <h4 className="text-gray-900 font-black text-sm mb-3">Dr. {doctorName} has joined the meet</h4>
                    <div className="flex gap-2">
                        <button
                            onClick={onJoin}
                            className="flex-1 bg-brand-purple text-white text-[11px] font-black py-2.5 rounded-xl hover:bg-brand-purple/90 transition-all shadow-lg shadow-brand-purple/20"
                        >
                            Join Now
                        </button>
                        <button
                            onClick={onClose}
                            className="px-3 bg-gray-100 text-gray-500 text-[11px] font-black py-2.5 rounded-xl hover:bg-gray-200 transition-all"
                        >
                            Dismiss
                        </button>
                    </div>
...
const UserDashboard: React.FC<UserDashboardProps> = ({ onSignOut, patient, onUpdatePatient, userName }) => {
    ...
    const handleActionCenterClick = (actionType: any) => {
    ...
    const handleModuleNavigate = (target: FocusMode) => setFocusMode(target);
    ...
    const handleMedicalHistoryComplete = (data: any) => {
        ...
        onUpdatePatient(patient.id, intakeEvent, { ...data, status: 'Assessment Review' });
        closeFocusMode();
    };
    ...
    const handlePsychProfileComplete = (data: any) => {
        const intakeEvent: Omit<TimelineEvent, 'id' | 'date'> = {
            ...
            description: 'Patient completed behavioral and mental health assessment.'
        };
        onUpdatePatient(patient.id, intakeEvent, { ...data });
        closeFocusMode();
    };

    const handleLabScheduled = (data: any) => {
        const labEvent: Omit<TimelineEvent, 'id' | 'date'> = {
            type: 'Labs',
            title: 'Initial Metabolic Labs Scheduled',
            description: `Lab visit booked at ${data.centerName}. Results will be available in 3-5 days.`,
            context: { labDateTime: data.dateTime, center: data.centerName }
        };
        onUpdatePatient(patient.id, labEvent, { status: 'Awaiting Labs' });
        closeFocusMode();
    };

    const handleConsultScheduled = (data: any) => {
        const consultEvent: Omit<TimelineEvent, 'id' | 'date'> = {
            type: 'Consultation',
            title: 'Doctor Consultation Scheduled',
            description: `Video appointment confirmed for ${data.dateTime}. You will receive a link to join.`,
            context: { consultDateTime: data.dateTime }
        };
        onUpdatePatient(patient.id, consultEvent, { status: 'Ready for Consult' });
        closeFocusMode();
    };
    ...
    const handleEndCall = (data?: { transcript: string; summary: string }) => {
        const transcriptEvent = {
            ...
            description: data
                ? data.summary
                : 'Doctor reviewed metabolic profile. Patient cleared for GLP-1 therapy.',
            doctor: patient.careTeam.physician,
            context: data ? { transcript: data.transcript, summary: data.summary } : undefined
        };
    ...
    const renderDashboardOverview = () => (
        <div className="animate-fade-in">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
                ...
                />
                <MedicationSummary patient={patient} />
            </div>
            <TreatmentProgressSection patient={patient} onUpdatePatient={onUpdatePatient} />
            <div className="mb-4">
                <h2 className="text-2xl font-black text-gray-900 tracking-tight flex items-center gap-3">
                    <span className="w-8 h-8 rounded-lg bg-emerald-100 flex items-center justify-center text-sm shadow-inner">⚡</span>
                    Care Modules
                </h2>
            </div>
        ...
        switch (currentView) {
            ...
            case 'consultations':
                return (
                    <ConsultationsScreen
                        patient={patient}
                        isDoctorInCall={isDoctorInCall}
                        onJoinCall={() => setFocusMode('telehealth')}
                    />
                );
            case 'dashboard':
    ...
    const renderOverlays = () => (
        <>
            ...
            {/* Modals */}
            <ModalWrapper isOpen={focusMode === 'schedule_consult'} onClose={closeFocusMode} title="Schedule Consultation">
                <ConsultationScheduler onSchedule={handleConsultScheduled} minBookingNoticeDays={2} minDate={minConsultDate} />
            </ModalWrapper>

            <ModalWrapper isOpen={focusMode === 'schedule_labs'} onClose={closeFocusMode} title="Book Lab Visit">
                <LabScheduler onSchedule={handleLabScheduled} />
            </ModalWrapper>

            <ModalWrapper isOpen={focusMode === 'telehealth'} onClose={() => handleEndCall()} title="Telehealth Session" maxWidth="max-w-4xl">
                <ConsultationCall
            ...
            <ModalWrapper isOpen={focusMode === 'view_plan'} onClose={closeFocusMode} title="Treatment Plan Details" maxWidth="max-w-xl">
                <PrescriptionView patient={patient} />
            </ModalWrapper>

            <ModalWrapper isOpen={focusMode === 'intake_medical_form'} onClose={closeFocusMode} title="Medical History Form">
                <DigitalIntake type="medical" patient={patient} onComplete={handleMedicalHistoryComplete} />
            </ModalWrapper>
